...
switch ($go)
{
	...
	$name_aystopper = "Новые вещи";
	break;
	
	case "procat":
	$query_aystopper = "WHERE visible='1' AND procat = '1'";
	$name_aystopper = "Вещи на прокат";
	break;
	
	case "b_y":
	$query_aystopper = "WHERE visible='1' AND b_y = '1'";
	$name_aystopper = "Б/У вещи";
	break;
	
	case "sale":
	$query_aystopper = "WHERE visible='1' AND sale = '1'";
	$name_aystopper = "Акции";
	break;	
	...
	}}else{$go='products_id DESC';
	$name_aystopper='Категория товаров не найдена';}
...
<ul id="sorting-list">
<li a href="view_aystopper.php?go=<?php $go ?>&price-asc">По возростанию цены</li>
<li a href="view_aystopper.php?go=<?php $go ?>&price-desc">По убыванию цены</li>
<li a href="view_aystopper.php?go=<?php $go ?>&brand">По алфавиту</li>
</ul>
...
$query = "SELECT * FROM table_products $query_aystopper";
...
$db = mysqli_connect("192.168.1.34","lilia","password", "db_shop") or die ("Error");
#$db->query( "SET CHARSET utf8" );
$query = "SELECT * FROM table_products $query_aystopper ORDER BY $sorting";
$result = mysqli_query($db, $query);
$row_cnt = mysqli_num_rows($result);
